...
It is a random variable with expected value ?? and standard error ??/sqrt(N).
"
#===============================================================================
...
res <- replicate(B,{
  ...
  se <- sd(X)/sqrt(N)
  z = qnorm(0.975)
  interval <- c(X_hat-z*se,X_hat+z*se)
  between(mu,X_hat-z*se,X_hat+z*se)
})
...
res <- polls %>% group_by(pollster) %>%
  summarize(s = sd(spread), avg = mean(spread), N=n())
