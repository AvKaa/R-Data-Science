...
ref <- NHANES %>% filter(AgeDecade == " 20-29" & Gender == "female") %>% summarize(average = mean(BPSysAve,na.rm = TRUE), ref = sd(BPSysAve,na.rm = TRUE))
...
"
Q7.
We S %>% group_by(AgeDecade, Gender) %>% summarize(average = mean(BPSysAve,na.rm = TRUE), standard_deviation = sd(BPSysAve,na.rm = TRUE))can actually combine both of these summaries into a single line of code. This is because group_by permits us to group by more than one variable.
...
library(NHANES)
data(NHANES)
NHANES %>% filter(Gender == "male", AgeDecade == " 40-49") %>% group_by(Race1) %>% summarize(average = mean(BPSysAve,na.rm = T), standard_deviation = sd(BPSysAve,na.rm = T)) %>% arrange(average)
